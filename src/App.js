...
const App = () => {
  ...
  const getAllProjects = async () => {
    try {
      ...
      if (contract.options.address) {
        const pr = await contract.methods.returnAllProjects().call();
        pr.forEach(async (projectAddress) => {
          const projectInst = crowdfundProject(projectAddress);
          const projectData = await projectInst.methods.getInfo().call();
          const projectInfo = { ...projectData, isLoading: false, contract: projectInst };
  ...
  useEffect(() => {
    if (web3 !== undefined && accounts !== undefined && contract) {
      // Ensure contract address is set before calling getAllProjects
      if (contract.options.address) {
        getAllProjects();
      } else {
        console.error("Contract address not set yet");
